...
const fs = require("fs");
const gedcom = require("./");
const parse = gedcom.parse;
const d3ize = gedcom.d3ize;

function fixture(t, name) {
  var result = parse(
    fs.readFileSync(__dirname + "/fixture/" + name + ".ged", "utf8")
  );
  if (process.env.UPDATE) {
    fs.writeFileSync(
      __dirname + "/fixture/" + name + ".json",
...
function fixture_d3ize(t, name) {
  ...
  if (process.env.UPDATE) {
    fs.writeFileSync(
      __dirname + "/fixture/" + name + ".d3.json",
      JSON.stringify(result, null, 2)
    );
  }
